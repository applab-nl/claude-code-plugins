  ...
  "node_modules/uri-js/dist/es5/uri.all.js"(exports, module) {
    ...
    (function(global3, factory) {
      ...
      function pctEncChar(chr) {
        ...
        var e = void 0;
        if (c3 < 16) e = "%0" + c3.toString(16).toUpperCase();
        else if (c3 < 128) e = "%" + c3.toString(16).toUpperCase();
        else if (c3 < 2048) e = "%" + (c3 >> 6 | 192).toString(16).toUpperCase() + "%" + (c3 & 63 | 128).toString(16).toUpperCase();
        else e = "%" + (c3 >> 12 | 224).toString(16).toUpperCase() + "%" + (c3 >> 6 & 63 | 128).toString(16).toUpperCase() + "%" + (c3 & 63 | 128).toString(16).toUpperCase();
        return e;
      }
...
var require_compile = __commonJS({
  "node_modules/ajv/lib/compile/index.js"(exports, module) {
    ...
    function compile(schema, root, localRefs, baseId) {
      ...
      if (c3.compiling) return compilation.callValidate = callValidate;
    ...
    function compIndex(schema, root, baseId) {
      for (var i2 = 0; i2 < this._compilations.length; i2++) {
        ...
        if (c3.schema == schema && c3.root == root && c3.baseId == baseId) return i2;
      }
      return -1;
    }
...
var require_windows = __commonJS({
  "node_modules/isexe/windows.js"(exports, module) {
    ...
    function isexe(path12, options, cb) {
      fs6.stat(path12, function(er, stat) {
        cb(er, er ? false : checkStat(stat, path12, options));
      });
    }
    function sync(path12, options) {
      return checkStat(fs6.statSync(path12), path12, options);
    }
  }
});
...
var require_mode = __commonJS({
  "node_modules/isexe/mode.js"(exports, module) {
    ...
    function sync(path12, options) {
      return checkStat(fs6.statSync(path12), options);
    }
...
var require_package = __commonJS({
  "node_modules/node-persist/package.json"(exports, module) {
    module.exports = {
      name: "node-persist",
      version: "4.0.4",
...
var require_local_storage = __commonJS({
  "node_modules/node-persist/src/local-storage.js"(exports, module) {
    init_cjs_shims();
    var fs6 = __require("fs");
    var path12 = __require("path");
    ...
    var resolveDir = function(dir) {
      dir = path12.normalize(dir);
      if (path12.isAbsolute(dir)) {
        return dir;
      }
      return path12.join(process.cwd(), dir);
    };
    ...
    LocalStorage.prototype = {
      ...
      setOptions: function(userOptions2) {
        let options = {};
        if (!userOptions2) {
          options = defaults;
        } else {
          for (let key in defaults) {
            if (userOptions2.hasOwnProperty(key)) {
      ...
      getDatumPath: function(key) {
        return path12.join(this.options.dir, sha256(key));
      },
      del: function(key) {
              ...
              fs6.readdir(dir, async (err, arr) => {
                ...
                try {
                  for (let currentFile of arr) {
                    if (currentFile[0] !== ".") {
                      data.push(await this.limit(() => this.readFile(path12.join(this.options.dir, currentFile))));
                    }
                  }
                } catch (err2) {
                  reject(err2);
      ...
      readFile: function(file, options = {}) {
        return this.limit(() => new Promise((resolve, reject) => {
          fs6.readFile(file, this.options.encoding, (err, text) => {
            if (err) {
              if (err.code === "ENOENT") {
      ...
      writeFile: async function(file, content) {
        return this.limit(() => new Promise((resolve, reject) => {
          fs6.writeFile(file, this.stringify(content), this.options.encoding, (err) => {
            if (err) {
              return reject(err);
            }
...
var require_node_persist = __commonJS({
  "node_modules/node-persist/src/node-persist.js"(exports, module) {
    init_cjs_shims();
    var LocalStorage = require_local_storage();
    (function(nodePersist) {
      nodePersist.create = function(userOptions2) {
        return LocalStorage(userOptions2);
      };
      nodePersist.init = async function(userOptions2) {
        const localStorage2 = nodePersist.defaultInstance = nodePersist.create(userOptions2);
        let ret = await localStorage2.init(userOptions2);
        mixin(nodePersist, localStorage2, { skip: ["init", "initSync", "create"] });
        return ret;
      };
      nodePersist.initSync = function(userOptions2) {
        const localStorage2 = nodePersist.defaultInstance = nodePersist.create(userOptions2);
        let ret = localStorage2.initSync(userOptions2);
        mixin(nodePersist, localStorage2, { skip: ["init", "initSync", "create"] });
        return ret;
      };
...
external_exports.union([EmptyResultSchema, CreateMessageResultSchema, ElicitResultSchema, ListRootsResultSchema]);
external_exports.union([PingRequestSchema, CreateMessageRequestSchema, ElicitRequestSchema, ListRootsRequestSchema]);
external_exports.union([
  CancelledNotificationSchema,
  ProgressNotificationSchema,
...
var StdioServerTransport = class {
  constructor(_stdin = process5__default.default.stdin, _stdout = process5__default.default.stdout) {
    this._stdin = _stdin;
...
init_cjs_shims();
var CreateWorktreeSchema = external_exports.object({
  repository: external_exports.string().min(1, "Repository path is required"),
  branch: external_exports.string().min(1, "Branch name is required"),
  name: external_exports.string().optional(),
  baseBranch: external_exports.string().optional()
});
var LaunchSessionSchema = external_exports.object({
  worktreePath: external_exports.string().min(1, "Worktree path is required"),
  prompt: external_exports.string().min(1, "Prompt is required"),
  contextFiles: external_exports.array(external_exports.string()).optional(),
  agentName: external_exports.string().optional(),
  terminalApp: external_exports.enum(["warp", "iterm2", "terminal", "custom"]).optional()
});
var ListWorktreesSchema = external_exports.object({
  repository: external_exports.string().optional()
});
var CleanupWorktreeSchema = external_exports.object({
  worktreePath: external_exports.string().min(1, "Worktree path is required"),
  force: external_exports.boolean().optional().default(false),
  removeBranch: external_exports.boolean().optional().default(false)
});
var GetSessionStatusSchema = external_exports.object({
  sessionId: external_exports.string().min(1, "Session ID is required")
});
var CreateTerminalSchema = external_exports.object({
  shell: external_exports.string().optional(),
...
async function validateDirectory(dirPath) {
  try {
    const stats = await fs4__default.default.stat(dirPath);
...
async function validateGitRepository(repoPath) {
  try {
    const gitDir = path3__default.default.join(repoPath, ".git");
...
function normalizePath(filePath) {
  return path3__default.default.resolve(path3__default.default.normalize(filePath));
}
function sanitizeFilename(name) {
  return name.replace(/[^a-zA-Z0-9-_]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "").toLowerCase();
}
function generateWorktreeName(repoPath, branch) {
  const repoName = path3__default.default.basename(repoPath);
...
function bufferToString(input) {
  return (Array.isArray(input) ? buffer.Buffer.concat(input) : input).toString("utf-8");
}
...
var STANDARD_STREAMS = [process5__default.default.stdin, process5__default.default.stdout, process5__default.default.stderr];
...
var verboseDefault = util$1.debuglog("execa").enabled ? "full" : "none";
var DEFAULT_OPTIONS = {
  lines: false,
  buffer: true,
...
var escapeLines = (lines) => util$1.stripVTControlCharacters(lines).split("\n").map((line) => escapeControlCharacters(line)).join("\n");
...
var quoteString = (escapedArgument) => {
  ...
  return process5.platform === "win32" ? `"${escapedArgument.replaceAll('"', '""')}"` : `'${escapedArgument.replaceAll("'", "'\\''")}'`;
};
...
function isUnicodeSupported() {
  const { env } = process5__default.default;
  const { TERM, TERM_PROGRAM } = env;
  if (process5__default.default.platform !== "win32") {
    return TERM !== "linux";
  }
...
var serializeVerboseMessage = (message) => {
  const messageString = typeof message === "string" ? message : util$1.inspect(message);
...
init_cjs_shims();
var getStartTime = () => process5.hrtime.bigint();
var getDurationMs = (startTime) => Number(process5.hrtime.bigint() - startTime) / 1e6;
...
function traversePathUp(startPath) {
  return {
    *[Symbol.iterator]() {
      let currentPath = path3__default.default.resolve(toPath(startPath));
      let previousPath;
      while (previousPath !== currentPath) {
        yield currentPath;
        previousPath = currentPath;
        currentPath = path3__default.default.resolve(currentPath, "..");
      }
    }
  };
}
...
// node_modules/npm-run-path/index.js
var npmRunPath = ({
  cwd = process5__default.default.cwd(),
  path: pathOption = process5__default.default.env[pathKey()],
  preferLocal = true,
  execPath: execPath2 = process5__default.default.execPath,
  addExecPath = true
} = {}) => {
  const cwdPath = path3__default.default.resolve(toPath(cwd));
  const result = [];
  const pathParts = pathOption.split(path3__default.default.delimiter);
  if (preferLocal) {
  ...
  return pathOption === "" || pathOption === path3__default.default.delimiter ? `${result.join(path3__default.default.delimiter)}${pathOption}` : [...result, pathOption].join(path3__default.default.delimiter);
};
var applyPreferLocal = (result, pathParts, cwdPath) => {
  for (const directory of traversePathUp(cwdPath)) {
    const pathPart = path3__default.default.join(directory, "node_modules/.bin");
...
var applyExecPath = (result, pathParts, execPath2, cwdPath) => {
  const pathPart = path3__default.default.resolve(cwdPath, toPath(execPath2), "..");
  if (!pathParts.includes(pathPart)) {
    result.push(pathPart);
  }
};
var npmRunPathEnv = ({ env = process5__default.default.env, ...options } = {}) => {
  env = { ...env };
...
var getSendMethod = (anyProcess) => {
  ...
  const sendMethod = util$1.promisify(anyProcess.send.bind(anyProcess));
...
var handleNodeOption = (file, commandArguments, {
  ...
  nodeOptions = process5.execArgv.filter((nodeOption) => !nodeOption.startsWith("--inspect")),
  cwd,
  ...
  const resolvedNodePath = path3__default.default.resolve(cwd, normalizedNodePath);
  const newOptions = {
    ...options,
  ...
  if (path3__default.default.basename(file, ".exe") === "node") {
...
var normalizeCwd = (cwd = getDefaultCwd()) => {
  ...
  return path3__default.default.resolve(cwdString);
};
var getDefaultCwd = () => {
  try {
    return process5__default.default.cwd();
  } catch (error) {
...
var normalizeOptions = (filePath, rawArguments, rawOptions) => {
  ...
  if (process5__default.default.platform === "win32" && path3__default.default.basename(file, ".exe") === "cmd") {
...
var getEnv = ({ env: envOption, extendEnv, preferLocal, node, localDirectory, nodePath }) => {
  const env = extendEnv ? { ...process5__default.default.env, ...envOption } : envOption;
  if (preferLocal || node) {
...
var serializeIpcMessage = (ipcMessage) => typeof ipcMessage === "string" ? ipcMessage : util$1.inspect(ipcMessage);
...
var getStandardStreamDirection = (value) => {
  if ([0, process5__default.default.stdin].includes(value)) {
    return "input";
  }
  if ([1, 2, process5__default.default.stdout, process5__default.default.stderr].includes(value)) {
    return "output";
  }
};
...
init_cjs_shims();
var pushChunks = util$1.callbackify(async (getChunks, state, getChunksArguments, transformStream) => {
...
var destroyTransform = util$1.callbackify(async ({ currentIterable }, error) => {
...
var writeToFiles = (serializedResult, stdioItems, outputFiles) => {
  for (const { path: path12, append: append2 } of stdioItems.filter(({ type }) => FILE_TYPES.has(type))) {
    const pathString = typeof path12 === "string" ? path12 : path12.toString();
    if (append2 || outputFiles.has(pathString)) {
      fs.appendFileSync(path12, serializedResult);
    } else {
      outputFiles.add(pathString);
      fs.writeFileSync(path12, serializedResult);
    }
  }
};
...
var getIpcExport = () => {
  const anyProcess = process5__default.default;
  const isSubprocess = true;
  const ipc = process5__default.default.channel !== void 0;
  return {
...
var addPropertiesAsync = {
  ...
  output: {
    ...
    filePath: ({ value: { file, append: append2 } }) => ({ stream: fs.createWriteStream(file, append2 ? { flags: "a" } : {}) }),
...
var {
   ...
   */
  onExit} = signalExitWrap(processOk(process10) ? new SignalExit(process10) : new SignalExitFallback());
...
var unpipeOnSignalAbort = async (unpipeSignal, { sourceStream, mergedStream, fileDescriptors, sourceOptions, startTime }) => {
  await util$1.aborted(unpipeSignal, sourceStream);
...
var createReadable = ({ subprocess, concurrentStreams, encoding }, { from, binary: binaryOption = true, preserveNewlines = true } = {}) => {
  ...
  const readable2 = new stream.Readable({
    read,
    destroy: util$1.callbackify(onReadableDestroy.bind(void 0, { subprocessStdout, subprocess, waitReadableDestroy })),
...
var createWritable = ({ subprocess, concurrentStreams }, { to } = {}) => {
  ...
  const writable2 = new stream.Writable({
    ...
    destroy: util$1.callbackify(onWritableDestroy.bind(void 0, {
      subprocessStdin,
      subprocess,
      waitWritableFinal,
      waitWritableDestroy
...
var getWritableMethods = (subprocessStdin, subprocess, waitWritableFinal) => ({
  ...
  final: util$1.callbackify(onWritableFinal.bind(void 0, subprocessStdin, subprocess, waitWritableFinal))
});
...
var createDuplex = ({ subprocess, concurrentStreams, encoding }, { from, to, binary: binaryOption = true, preserveNewlines = true } = {}) => {
  ...
  const duplex2 = new stream.Duplex({
    read,
    ...getWritableMethods(subprocessStdin, subprocess, waitWritableFinal),
    destroy: util$1.callbackify(onDuplexDestroy.bind(void 0, {
      subprocessStdout,
      subprocessStdin,
      subprocess,
      waitReadableDestroy,
...
var SimpleLogger = class {
  ...
  formatMessage(level, message, args) {
    ...
    const argsStr = args.length > 0 ? " " + args.map((a2) => JSON.stringify(a2)).join(" ") : "";
...
var GitService = class {
  ...
  async validateRepository() {
    ...
    const isValid2 = await validateGitRepository(this.repoPath);
    if (!isValid2) {
  ...
  async branchExists(branchName) {
    ...
    const exists2 = branches.some((b) => b.name === branchName);
    logger.debug(`Branch ${branchName} exists: ${exists2}`);
    return exists2;
  }
  /**
  ...
  async createBranch(branchName, baseBranch) {
    await this.validateRepository();
    const exists2 = await this.branchExists(branchName);
    if (exists2) {
  ...
  async worktreeExists(worktreePath) {
    ...
    const exists2 = worktrees.some((w) => w.path === normalizedPath);
    logger.debug(`Worktree ${worktreePath} exists: ${exists2}`);
    return exists2;
  }
  /**
   * Create a new worktree
   */
  async createWorktree(branchName, worktreeName, baseBranch) {
    await this.validateRepository();
    const name = worktreeName || generateWorktreeName(this.repoPath, branchName);
    const parentDir = path3__default.default.dirname(this.repoPath);
    const worktreePath = path3__default.default.join(parentDir, name);
  ...
  async removeWorktree(worktreePath, force = false) {
    ...
    const exists2 = await this.worktreeExists(normalizedPath);
    if (!exists2) {
      throw new WorktreeError(
   ...
   */
  async executeInitScripts(worktreePath, sourceRepo) {
    const initDir = path3__default.default.join(worktreePath, ".worktree-init");
    logger.debug(`Looking for init scripts in: ${initDir}`);
    try {
      await fs4__default.default.access(initDir);
    } catch {
      logger.debug("No .worktree-init directory found");
      return [];
    }
    const files = await fs4__default.default.readdir(initDir);
    const scripts = files.filter((f) => f.endsWith(".sh")).sort();
    logger.info(`Found ${scripts.length} initialization scripts`);
    logger.debug(`Init scripts:`, scripts);
    const results = [];
    for (const script of scripts) {
      const scriptPath = path3__default.default.join(initDir, script);
      const startTime = Date.now();
      try {
        logger.debug(`Making ${script} executable`);
        await fs4__default.default.chmod(scriptPath, 493);
        ...
        const result = await execa(scriptPath, [sourceRepo], {
          ...
          all: true
          // Capture both stdout and stderr into result.all
        });
        if (result.all) {
    ...
    for (const line of lines) {
      if (line.startsWith("worktree ")) {
        ...
        current = {
          ...
          // We don't have creation time from git
        };
...
function createGitService(repositoryPath) {
...
function expandTilde(filepath) {
  if (filepath.startsWith("~/") || filepath === "~") {
    return path3__default.default.join(os__default.default.homedir(), filepath.slice(1));
  }
  return filepath;
}
var StateService = class _StateService {
  ...
  constructor(storageDir) {
    const expandedDir = storageDir ? expandTilde(storageDir) : void 0;
    this.storageDir = expandedDir || path3__default.default.join(
  ...
  static getInstance(storageDir) {
    if (!_StateService.instance) {
      _StateService.instance = new _StateService(storageDir);
    }
    return _StateService.instance;
  }
  /**
  ...
  async init() {
    ...
    logger2.debug("Initializing state service", { dir: this.storageDir });
    await import_node_persist.default.init({
      dir: this.storageDir,
      stringify: JSON.stringify,
      parse: JSON.parse,
      encoding: "utf8",
      expiredInterval: 24 * 60 * 60 * 1e3
      // Clean up once per day
    });
    this.initialized = true;
    logger2.info("State service initialized successfully");
  }
  /**
  ...
  async saveWorktree(worktree) {
    ...
    logger2.debug("Saving worktree", { path: worktree.path, branch: worktree.branch });
    await import_node_persist.default.setItem(key, {
      ...worktree,
  ...
  async getWorktree(worktreePath) {
    ...
    const data = await import_node_persist.default.getItem(key);
    if (!data) {
      return null;
    }
    return {
  ...
  async listWorktrees(repositoryFilter) {
    await this.ensureInitialized();
    const keys = await import_node_persist.default.keys();
    const worktreeKeys = keys.filter((k) => k.startsWith("worktree:"));
    const worktrees = await Promise.all(
      worktreeKeys.map(async (key) => {
        const data = await import_node_persist.default.getItem(key);
        if (!data) return null;
        return {
          ...data,
    ...
    if (repositoryFilter) {
      return validWorktrees.filter(
        (w) => w.repository === repositoryFilter
      );
    }
  ...
  async deleteWorktree(worktreePath) {
    await this.ensureInitialized();
    const key = this.getWorktreeKey(worktreePath);
    logger2.debug("Deleting worktree from state", { path: worktreePath });
    await import_node_persist.default.removeItem(key);
    return true;
  }
  /**
  ...
  async getSession(sessionId) {
    ...
    return {
      ...
      lastActivity: data.lastActivity ? new Date(data.lastActivity) : void 0
    };
  }
  /**
   * List all sessions
  ...
  async findSessionByWorktree(worktreePath) {
    ...
    const activeSessions = sessions.filter((s) => s.status === "active").sort((a2, b) => b.startedAt.getTime() - a2.startedAt.getTime());
  ...
  async updateSessionStatus(sessionId, status) {
    ...
    if (session) {
      ...
      if (status !== "active") {
        await this.updateWorktreeSession(session.worktreePath, void 0);
      }
    }
  }
  /**
  ...
  async deleteSession(sessionId) {
    ...
    const key = this.getSessionKey(sessionId);
    logger2.debug("Deleting session from state", { sessionId });
    await import_node_persist.default.removeItem(key);
    return true;
  }
  // =========================================================================
  ...
  async clearAll() {
    await this.ensureInitialized();
    logger2.warn("Clearing all state");
    await import_node_persist.default.clear();
  }
  /**
  ...
  async cleanupOldSessions(daysOld = 30) {
    ...
    for (const session of sessions) {
      if (session.status !== "active" && session.completedAt && session.completedAt < cutoffDate) {
        await this.deleteSession(session.sessionId);
        cleaned++;
      }
    }
    logger2.info(`Cleaned up ${cleaned} old sessions`);
    return cleaned;
  }
  // =========================================================================
  ...
  getWorktreeKey(worktreePath) {
    const normalized = path3__default.default.normalize(worktreePath);
...
function getStateService(storageDir) {
...
async function createWorktree(params) {
  ...
  logger3.info("Creating worktree", { repository, branch, name });
  ...
  const existing = existingWorktrees.find(
    (w) => w.branch === branch || name && w.path.endsWith(name)
  );
  if (existing) {
    logger3.info("Worktree already exists", { path: existing.path });
    return {
  ...
  logger3.debug(`Creating worktree with baseBranch: ${baseBranch || "current HEAD"}`);
  ...
  try {
    ...
    for (const result of scriptResults) {
      if (result.success) {
        ...
        logger3.warn(`Init script ${result.script} failed`, { error: result.error });
      }
    }
  } catch (error) {
    logger3.error("Failed to execute init scripts", error);
  }
  await stateService.saveWorktree(worktree);
  logger3.info("Worktree created successfully", {
    path: worktree.path,
...
var createWorktreeToolDefinition = {
...
async function findGitRepositories() {
  logger4.debug("Searching for git repositories in common locations");
  const repos = [];
  const cwd = process.cwd();
  const parent = path3__default.default.dirname(cwd);
  logger4.debug(`Checking current directory: ${cwd}`);
  logger4.debug(`Checking parent directory: ${parent}`);
  for (const dir of [cwd, parent]) {
    if (await validateGitRepository(dir)) {
      logger4.debug(`Found git repository: ${dir}`);
      repos.push(dir);
    }
  }
  try {
    logger4.debug(`Scanning sibling directories in: ${parent}`);
    const siblings = await fs4__default.default.readdir(parent);
    logger4.debug(`Found ${siblings.length} sibling directories`);
    for (const sibling of siblings) {
      const siblingPath = path3__default.default.join(parent, sibling);
      if (await validateGitRepository(siblingPath)) {
        logger4.debug(`Found git repository: ${siblingPath}`);
        repos.push(siblingPath);
      }
    }
  } catch (error) {
    logger4.debug("Error reading parent directory", error);
  }
  const uniqueRepos = Array.from(new Set(repos));
  logger4.debug(`Found ${uniqueRepos.length} unique repositories`);
  return uniqueRepos;
}
async function listWorktrees(params) {
  const validated = ListWorktreesSchema.parse(params);
  const { repository } = validated;
  logger4.info("Listing worktrees", { repository });
  const stateService = getStateService();
  await stateService.init();
  const repositories = repository ? [repository] : await findGitRepositories();
  if (repositories.length === 0) {
    logger4.warn("No git repositories found");
    return { worktrees: [] };
  }
  logger4.debug(`Searching ${repositories.length} repositories`, { repositories });
  const allWorktrees = [];
  for (const repo of repositories) {
    try {
      logger4.debug(`Processing repository: ${repo}`);
      const gitService = createGitService(repo);
      await gitService.validateRepository();
      const worktrees = await gitService.listWorktrees();
      logger4.debug(`Found ${worktrees.length} worktrees in ${repo}`);
      for (const worktree of worktrees) {
        logger4.debug(`Checking for session in worktree: ${worktree.path}`);
        const session = await stateService.findSessionByWorktree(worktree.path);
        if (session) {
          logger4.debug(`Found session ${session.sessionId} for worktree ${worktree.path}`);
          worktree.sessionId = session.sessionId;
        }
      }
      allWorktrees.push(...worktrees);
    } catch (error) {
      logger4.warn(`Failed to list worktrees for ${repo}`, error);
    }
  }
  logger4.info(`Found ${allWorktrees.length} worktrees total`);
  return { worktrees: allWorktrees };
}
var listWorktreesToolDefinition = {
  name: "list_worktrees",
...
async function terminateProcess(pid) {
  ...
  try {
    ...
    logger5.debug("Waiting 2s for graceful shutdown");
    ...
    logger5.warn(`Failed to terminate process ${pid}`, error);
    return false;
  }
}
async function findParentRepository(worktreePath) {
  try {
    ...
    throw new Error(
      `Could not find parent repository for worktree: ${error.message}`
    );
  }
}
async function cleanupWorktree(params) {
  ...
  for (const session of sessions) {
    if (session.status === "active") {
      ...
      if (terminated) {
        ...
        sessionsTerminated++;
        logger5.debug(`Session ${session.sessionId} terminated`);
      }
    } else {
...
var cleanupWorktreeToolDefinition = {
  name: "cleanup_worktree",
  description: "Remove a git worktree and clean up associated resources including active sessions. Optionally removes the git branch as well.",
  inputSchema: {
    type: "object",
    properties: {
      worktreePath: {
...
var DEFAULT_TERMINAL_CONFIG = {
  app: "warp",
  // Use Warp by default
  detectOrder: ["warp", "iterm2", "terminal"]
  // Fallback order
};
var TerminalService = class {
  ...
  async detectAvailableTerminals() {
    logger6.debug("Detecting available terminal applications");
    const available = [];
    logger6.debug("Checking for Warp");
    if (await this.isWarpInstalled()) {
      logger6.debug("Warp is installed");
      available.push("warp");
    }
    logger6.debug("Checking for iTerm2");
    if (await this.isITerm2Installed()) {
      logger6.debug("iTerm2 is installed");
      available.push("iterm2");
    }
    if (process.platform === "darwin") {
      logger6.debug("Terminal.app is available (macOS)");
      available.push("terminal");
    }
    logger6.debug("Detected terminal applications", { available });
    return available;
  }
  /**
  ...
  async getBestTerminal() {
    logger6.debug("Determining best terminal to use", { configuredApp: this.config.app });
    if (this.config.app === "custom" && this.config.customCommand) {
      logger6.debug("Using custom terminal command");
      return "custom";
    }
    if (this.config.app !== "custom") {
      logger6.debug(`Checking if configured app ${this.config.app} is available`);
      const available2 = await this.detectAvailableTerminals();
      if (available2.includes(this.config.app)) {
        logger6.debug(`Using configured terminal: ${this.config.app}`);
        return this.config.app;
      }
      logger6.debug(`Configured app ${this.config.app} not available, falling back to detection order`);
    }
    const detectOrder = this.config.detectOrder || ["warp", "iterm2", "terminal"];
    logger6.debug("Falling back to detection order", { detectOrder });
    const available = await this.detectAvailableTerminals();
    for (const app of detectOrder) {
      if (available.includes(app)) {
        logger6.info(`Using terminal: ${app}`);
        return app;
      }
    }
    logger6.error("No terminal application found");
  ...
  async createTerminal(params) {
    logger6.debug("Creating terminal with params", {
      cwd: params.cwd,
      title: params.title,
      hasCommand: !!params.command
    });
    const terminalApp = await this.getBestTerminal();
    logger6.debug(`Selected terminal app: ${terminalApp}`);
    try {
      let result;
      switch (terminalApp) {
        case "warp":
          logger6.debug("Creating Warp terminal");
          result = await this.createWarpTerminal(params);
          break;
        case "iterm2":
          logger6.debug("Creating iTerm2 terminal");
          result = await this.createITerm2Terminal(params);
          break;
        case "terminal":
          logger6.debug("Creating macOS Terminal");
          result = await this.createMacTerminal(params);
          break;
        case "custom":
          logger6.debug("Creating custom terminal");
      ...
      logger6.info("Terminal created successfully", {
        app: result.app,
        pid: result.pid
      });
      return result;
    } catch (error) {
      logger6.error("Failed to create terminal", error);
      throw new TerminalError(
  ...
  async executeInTerminal(instance, command) {
    logger6.info("Execute command in terminal", {
      app: instance.app,
      pid: instance.pid,
      command
    });
  ...
  async isWarpInstalled() {
    try {
      ...
      logger6.debug(`Checking for Warp at: ${warpPath}`);
      await fs4__default.default.access(warpPath);
      logger6.debug("Warp is installed");
      return true;
    } catch {
      logger6.debug("Warp not found");
      return false;
    }
  }
  async createWarpTerminal(params) {
    ...
    if (command && cwd) {
      try {
        logger6.debug("Creating temporary launch script for Warp");
        const claudeDir = path3__default.default.join(cwd, ".claude");
        await fs4__default.default.mkdir(claudeDir, { recursive: true });
...
`;
        ...
        await fs4__default.default.writeFile(scriptPath, scriptContent, { mode: 493 });
        const uri = `warp://action/new_tab?path=${encodeURIComponent(scriptPath)}&`;
        logger6.debug(`Opening Warp with script URI: ${uri}`);
        await execa("open", [uri]);
        logger6.info("Warp terminal created with command execution via script");
        return {
          pid: -1,
          app: "warp",
          success: true
        ...
        logger6.error("Failed to create Warp launch script", error);
        logger6.warn("Falling back to basic Warp tab without command execution");
      ...
      const uri = cwd ? `warp://action/new_tab?path=${encodeURIComponent(cwd)}` : "warp://action/new_tab";
      logger6.debug(`Opening Warp with URI: ${uri}`);
      await execa("open", [uri]);
      return {
        pid: -1,
        app: "warp",
  ...
  async isITerm2Installed() {
    try {
      ...
      logger6.debug(`Checking for iTerm2 at: ${iterm2Path}`);
      await fs4__default.default.access(iterm2Path);
      logger6.debug("iTerm2 found");
      return true;
    } catch {
      logger6.debug("iTerm2 not found");
      return false;
    }
  }
  async createITerm2Terminal(params) {
    ...
    logger6.debug(`Using iTerm2 profile: ${profile}`);
    const script = `
    ...
    `;
    logger6.debug("Executing iTerm2 AppleScript");
    logger6.debug("AppleScript command:", script);
    await execa("osascript", ["-e", script]);
    logger6.debug("iTerm2 terminal created (no PID available)");
    return {
      pid: -1,
  ...
  async createMacTerminal(params) {
    ...
    if (profile) {
      const escapedProfile = profile.replace(/"/g, '\\"');
      logger6.debug(`Using Terminal.app settings: ${profile}`);
    ...
    `;
    logger6.debug("Executing Terminal.app AppleScript");
    logger6.debug("AppleScript command:", script);
    await execa("osascript", ["-e", script]);
    logger6.debug("Terminal.app terminal created (no PID available)");
    return {
      pid: -1,
  ...
  async createCustomTerminal(params) {
    ...
    logger6.debug("Executing custom terminal command", { command });
    const result = await execa("sh", ["-c", command]);
    const pid = result.pid ? parseInt(String(result.pid), 10) : -1;
    logger6.debug(`Custom terminal created with pid: ${pid}`);
    return {
      pid,
      app: "custom",
      success: true
...
function createTerminalService(config) {
  return new TerminalService(config);
}
function loadTerminalConfigFromEnv() {
  const config = {};
  ...
  if (process.env.TERMINAL_DETECT_ORDER) {
    const order = process.env.TERMINAL_DETECT_ORDER.split(",").map((s) => s.trim().toLowerCase()).filter((s) => s === "warp" || s === "iterm2" || s === "terminal");
    if (order.length > 0) {
...
var SessionService = class {
   ...
   */
  static generateSessionId(worktreePath) {
    const timestamp = Date.now();
  ...
  static async createPromptFile(worktreePath, prompt, agentName, contextFiles) {
    ...
    await fs4__default.default.mkdir(promptDir, { recursive: true });
...
`;
    ...
    await fs4__default.default.writeFile(promptFile, content, "utf-8");
  ...
  static async launchSession(params) {
    ...
    logger7.info("Launching Claude Code session", {
      worktreePath,
      agentName,
    ...
    const promptFile = await this.createPromptFile(
      ...
      prompt,
      agentName,
      // Pass agent name to be included in prompt
      contextFiles
    );
...
PROMPT_EOF
    ...
    const terminalService = createTerminalService(
      terminalApp ? { app: terminalApp } : void 0
    );
    try {
      logger7.debug("Creating terminal with command");
      const terminalResult = await terminalService.createTerminal({
        cwd: worktreePath,
        title: `Claude Code - ${path3__default.default.basename(worktreePath)}`,
        command: claudeCommand
      });
      logger7.debug("Terminal creation result:", {
      ...
      logger7.debug("Session saved to state");
  ...
  static async getSessionStatus(sessionId) {
    ...
    logger7.debug(`Checking if terminal process ${session.terminalPid} is alive`);
    const terminalAlive = await isProcessAlive(session.terminalPid);
    logger7.debug(`Terminal alive: ${terminalAlive}`);
    if (!terminalAlive && session.status === "active") {
      logger7.debug(`Terminal died, updating session status to terminated`);
  ...
  static async terminateSession(sessionId) {
    ...
    try {
      logger7.debug(`Sending SIGTERM to process ${session.terminalPid}`);
      process.kill(session.terminalPid, "SIGTERM");
      logger7.debug("Waiting 2s for graceful shutdown");
      await new Promise((resolve) => setTimeout(resolve, 2e3));
      const stillAlive = await isProcessAlive(session.terminalPid);
      if (stillAlive) {
        logger7.debug(`Process still alive, sending SIGKILL`);
        process.kill(session.terminalPid, "SIGKILL");
      } else {
        logger7.debug("Process terminated gracefully");
      }
      const stateService = getStateService();
      await stateService.init();
      await stateService.updateSessionStatus(sessionId, "terminated");
   ...
   */
  static async findSessionsByWorktree(worktreePath) {
    const stateService = getStateService();
    await stateService.init();
...
var createTerminalToolDefinition = {
  name: "create_terminal",
  description: "Create a new terminal window or tab with optional working directory, title, and initial command. This is a low-level tool primarily used by other tools like launch_session.",
  inputSchema: {
    type: "object",
    properties: {
      shell: {
