...
import { validateGitRepository, validateDirectory, generateWorktreeName, normalizePath, } from '../utils/validators.js';
...
export class GitService {
    ...
    async validateRepository() {
        ...
        if (!isValid) {
            throw new WorktreeError(`Not a valid git repository: ${this.repoPath}`, 'REPOSITORY_NOT_FOUND');
        }
    ...
    async listWorktrees() {
        ...
        try {
            ...
            throw new WorktreeError('Failed to list worktrees', 'GIT_ERROR', error);
        }
    }
    /**
    ...
    async createWorktree(branchName, worktreeName, baseBranch) {
        ...
        if (await this.worktreeExists(worktreePath)) {
            throw new WorktreeError(`Worktree already exists at ${worktreePath}`, 'WORKTREE_EXISTS');
        }
        // Check if directory already exists
        if (await validateDirectory(worktreePath)) {
            throw new WorktreeError(`Directory already exists at ${worktreePath}`, 'WORKTREE_EXISTS');
        }
        try {
            ...
            if (!newWorktree) {
                throw new WorktreeError('Worktree created but not found in list', 'GIT_ERROR');
            }
            ...
            throw new WorktreeError(`Failed to create worktree: ${error.message}`, 'GIT_ERROR', error);
        }
    }
    /**
     * Remove a worktree
    ...
    async removeWorktree(worktreePath, force = false) {
        ...
        if (!exists) {
            throw new WorktreeError(`Worktree not found at ${normalizedPath}`, 'WORKTREE_NOT_FOUND');
        }
        try {
            ...
            if (message.includes('contains modified or untracked files')) {
                throw new WorktreeError(`Worktree has uncommitted changes. Use force=true to override.`, 'WORKTREE_DIRTY', error);
            }
            logger.error('Failed to remove worktree', error);
            throw new WorktreeError(`Failed to remove worktree: ${message}`, 'GIT_ERROR', error);
        }
    }
    /**
     * Delete a branch
    ...
    async deleteBranch(branchName, force = false) {
        ...
        try {
            ...
            throw new WorktreeError(`Failed to delete branch: ${message}`, 'GIT_ERROR', error);
        }
    }
    /**
 ...
 */
export function createGitService(repositoryPath) {
