...
- **Rich Metadata**: Captures timestamp, session ID, and working directory
- **JSON Format**: Easy to parse and analyze

## How It Works

The plugin uses the `UserPromptSubmit` hook to capture prompts before they are processed. Each qualifying prompt is logged with metadata to `logs/prompts.json` in your current working directory.
...
Each entry in `logs/prompts.json` contains:
